...
def run_backtest(df, weights, buy_threshold, sell_threshold):
    ...
    Improvements over v1:
      1. Trend filter   — BUY only allowed when Close > SMA50 (uptrend)
      2. 3-candle confirmation — score must exceed threshold for 3 consecutive
         candles before a signal is fired (filters single-spike noise)
      3. Minimum hold of 24 candles (24h on 1H data) before exit is considered
    """
    MIN_HOLD_CANDLES = 24   # 1 day on 1H bars
    CONFIRM_CANDLES  = 3    # consecutive candles needed to confirm signal

    # ── Step 1: compute raw scores and trend flag for every candle
    records = []
    ...
    # ── Step 2: apply 3-candle confirmation → confirmed signal column
    confirmed = []
    for i in range(len(res)):
        if i < CONFIRM_CANDLES - 1:
            confirmed.append('WAIT')
            continue
        window = res.iloc[i - CONFIRM_CANDLES + 1 : i + 1]
        all_buy  = all(window['Score'] >= buy_threshold)  and all(window['Above_SMA50'])
        all_sell = all(window['Score'] <= sell_threshold)
        if all_buy:
            confirmed.append('BUY')
        elif all_sell:
            confirmed.append('SELL')
        else:
            confirmed.append('WAIT')

    res['Confirmed'] = confirmed

    # ── Step 3: trade simulation with min hold period
    trades = []
    in_trade      = False
    entry_price   = 0
    entry_date    = None
    ...
    for i in range(len(res)):
        ...
        if not in_trade and sig == 'BUY':
            ...
            held = i - entry_idx
            if held >= MIN_HOLD_CANDLES and sig == 'SELL':
...
with tab3:
    ...
    st.info("""**Active filters in this backtest:**
✅ Trend filter — BUY only when price is above SMA50 (uptrend confirmed)
✅ 3-candle confirmation — score must hold above threshold for 3 consecutive hours
✅ Minimum hold period — 24 hours before any exit is considered""")
    ...
    if st.button("▶️ Run Backtest", key="run_bt"):
        ...
        fig = make_subplots(rows=3, cols=1, shared_xaxes=True,
                            ...
                            subplot_titles=("MCX Gold Price (INR) with Confirmed Signals", "Score", "RSI"))
        ...
        # SMA50 overlay on price chart
        fig.add_trace(go.Scatter(
            x=df.index, y=df['SMA50'],
            line=dict(color='#f0d080', width=1, dash='dot'),
            name='SMA50'
        ), row=1, col=1)

        buy_dates  = bt_results[bt_results['Confirmed'] == 'BUY'].index
        sell_dates = bt_results[bt_results['Confirmed'] == 'SELL'].index

        # Only plot markers where dates exist in df
        buy_dates_valid  = buy_dates[buy_dates.isin(df.index)]
        sell_dates_valid = sell_dates[sell_dates.isin(df.index)]

        fig.add_trace(go.Scatter(x=buy_dates_valid,  y=df.loc[buy_dates_valid,  'Low_INR']  * 0.999,
                                  mode='markers', marker=dict(symbol='triangle-up',   size=10, color='#2ecc71'),
                                  name='BUY'), row=1, col=1)
        fig.add_trace(go.Scatter(x=sell_dates_valid, y=df.loc[sell_dates_valid, 'High_INR'] * 1.001,
        ...
        m1.metric("Confirmed BUY Signals",  buy_count)
        m2.metric("Confirmed SELL Signals", sell_count)
        ...
        if not trades_df.empty:
            ...
            e2.metric("Avg Hold (hrs)",  f"{avg_hold:.0f}h")
            ...
            trades_display['Hold_Hours']  = trades_display['Hold_Hours'].apply(lambda x: f"{x}h")
            ...
            trades_display.columns = ['Entry Date', 'Exit Date', 'Entry Price',
...
- **Win Rate:** {win_rate:.1f}% | **Total Trades:** {len(trades_df)} | **Risk:Reward:** {rr:.1f}x
- Current thresholds → Buy ≥ **{st.session_state.buy_thresh}** | Sell ≤ **{st.session_state.sell_thresh}**
- Filters applied: Trend (SMA50) ✅ | 3-candle confirmation ✅ | Min hold 24h ✅
